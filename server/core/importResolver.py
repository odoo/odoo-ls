...
def resolve_import_stmt(ls, source_file_symbol, parent_symbol, from_stmt, name_aliases, level, 
    ...
    for alias in name_aliases:
        ...
        name_symbol = next_symbol.get_symbol(ls, [last_part_name], excl=parent_symbol) #find the last part of the name
        if not name_symbol:
...
def _get_or_create_symbol(ls, symbol, names, file_symbol, asname, lineno, end_lineno):
    ...
    for branch in names:
        next_symbol = symbol.get_symbol(ls, [branch], excl=file_symbol)
        if not next_symbol:
            next_symbol = _resolve_new_symbol(ls, file_symbol, symbol, branch, asname, 
                                                lineno, end_lineno)
        elif next_symbol.archStatus == 0:
            from server.core.pythonArchBuilder import PythonArchBuilder
            parser = PythonArchBuilder(ls, symbol, next_symbol.paths[0]) #the symbol need a rebuild
            next_symbol = parser.load_arch()
        symbol = next_symbol
        if not symbol:
            break
    return symbol
