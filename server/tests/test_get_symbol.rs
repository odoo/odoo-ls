...
fn test_definition() {
    ...
    let partner_class_range = FileMgr::text_range_to_range(&mut session, &partner_class_file, partner_class_text_range);
    assert!(
    ...
    let compute_display_name_file_info = session.file_mgr().get_file_info(&compute_display_name_file).unwrap();
    let range = session.st().header_range(compute_display_name_sym, Some(&session.file_mgr()[compute_display_name_file_info])).unwrap();
    let compute_display_name_range = FileMgr::text_range_to_range(&mut session, &compute_display_name_file, range);
    assert!(
    ...
    let partner_name_range = FileMgr::text_range_to_range(&mut session, &partner_name_file, partner_name_text_range);
    assert!(
