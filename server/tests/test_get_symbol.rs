...
fn test_definition() {
    ...
    let compute_display_name_file_info = file_mgr.borrow().get_file_info(&compute_display_name_file).unwrap();
    let range = session.st().header_range(compute_display_name_sym, Some(compute_display_name_file_info)).unwrap();
