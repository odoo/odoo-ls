...
fn test_completion_in_template(session: &mut SessionInfo, fixtures: &Fixtures) {
    ...
    assert_no_completions(session, xml, "this.shout(|this.props.exclamations)", &[
        ...
        "arguments" // we see this binding because we evaluate template expression inside fake functions
    ]);
}
...
/// `@odoo/hoot-mock` is the `alias=` of a real `static/lib` file
fn test_odoo_scoped_module_paths(session: &mut SessionInfo, fixtures: &Fixtures) {
    ...
    assert_import_resolves_in(session, tests_js, "@odoo/hoot-mock", "web/static/lib/hoot/hoot-mock.js");
}
