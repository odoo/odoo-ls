...
pub type RecordCreationHookFn =
    fn(session: &mut SessionInfo, source_file: SymbolKey, record: &OdooDataRecord);
...
pub type FileUnloadHookFn = fn(session: &mut SessionInfo, file: SymbolKey);
...
pub fn on_file_unload(session: &mut SessionInfo, file: SymbolKey) {
