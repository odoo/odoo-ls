...
impl CsvArchBuilder {
    ...
    pub fn load_csv(&mut self, session: &mut SessionInfo, csv_symbol: Rc<RefCell<Symbol>>, content: &String) -> Vec<Diagnostic> {
        ...
        let model_name = model_name_pb.file_stem().unwrap().to_str().unwrap().to_string();
                    ...
                    if let Ok(result) = result {
                        ...
                        if let Some(mut record) = record {
                            if let Some(xml_id) = record.xml_id.as_ref() {
                                ...
                                let mut xml_module = csv_module.clone();
                                if id_split.len() == 2 {
    ...
    fn extract_record(&self, file_symbol: Weak<RefCell<Symbol>>, model_name: String, headers: &Vec<String>, record: &StringRecord) -> Option<OdooDataRecord> {
