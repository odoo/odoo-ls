...
impl FileInfo {
    ...
    pub fn update(&mut self, session: &mut SessionInfo, uri: &str, content: Option<&Vec<TextDocumentContentChangeEvent>>, version: Option<i32>, is_external: bool, force: bool) -> bool {
...
impl FileMgr {
    ...
    pub fn update_file_info(&mut self, session: &mut SessionInfo, uri: &str, content: Option<&Vec<TextDocumentContentChangeEvent>>, version: Option<i32>, force: bool) -> (bool, Rc<RefCell<FileInfo>>) {
        ...
        if (version.is_some() && version.unwrap() != -100) || !file_info.borrow().opened || force {
            ...
            drop(ep_mgr);
            updated = file_info_mut.update(session, uri, content, version, !is_part_of_ep, force);
            drop(file_info_mut);
        }
        (updated, return_info)
    }
