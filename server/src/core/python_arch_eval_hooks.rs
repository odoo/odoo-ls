...
impl PythonArchEvalHooks {
    ...
    fn on_base_model_eval(odoo: &mut SyncOdoo, symbol: Rc<RefCell<MainSymbol>>, file_symbol: Rc<RefCell<MainSymbol>>) {
        ...
        if !ids.is_empty() {
            ...
            let range = ids.last().unwrap().borrow().range();
    ...
    fn on_env_eval(odoo: &mut SyncOdoo, symbol: Rc<RefCell<MainSymbol>>, file_symbol: Rc<RefCell<MainSymbol>>) {
        ...
        if !cursor_sym.is_empty() && !cr.is_empty() {
            ...
            let sym_ref = Rc::downgrade(cr_mut);
    ...
    fn _update_get_eval(odoo: &mut SyncOdoo, symbol: Rc<RefCell<MainSymbol>>, tree: Tree) {
        ...
        if return_sym.is_empty() {
            symbol.borrow_mut().not_found_paths_mut().push((BuildSteps::ARCH_EVAL, flatten_tree(&tree)));
