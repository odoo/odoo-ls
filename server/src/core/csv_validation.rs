...
use crate::{S, Sy, constants::{BuildStatus, BuildSteps, OYarn}, core::{diagnostics::{DiagnosticCode, create_diagnostic}}, features::csv_ast_utils::CsvAstUtils, oyarn, threads::SessionInfo};
...
impl CsvValidator {
    ...
    pub fn validate(&mut self, session: &mut SessionInfo, csv_symbol: Rc<RefCell<Symbol>>) {
        ...
        let Some(model) = session.sync_odoo.models.get(&model_name).cloned() else {
            ...
            csv_sym.set_build_status(BuildSteps::VALIDATION, BuildStatus::DONE);
                        ...
                        if member_sym.0.is_empty() {
                            ...
                            for sym in member_sym.0.iter() {
                                if sym.borrow().is_specific_field(session, &["Many2one", "One2many", "Many2many"]) {
                                    if header_elts.len() == 2 && header_elts[1] == "id" {
                            ...
                            header_is_xml.push(is_relational);
                        }
                        start = end + 1;
                    }
                }
            ...
            if !csv.headers.is_empty() && csv.headers[0] == "id" {
                for result in rdr.records() {
                    if let Ok(result) = result {
                        self.validate_record(session, csv_module, model_name.clone(), &header_is_xml, &result, &mut diagnostics);
                    }
                }
            }
        }
    ...
    fn validate_record(&self, session: &mut SessionInfo, csv_module: &Rc<RefCell<Symbol>>, model_name: OYarn, headers_is_xml: &Vec<bool>, record: &StringRecord, diagnostics: &mut Vec<Diagnostic>) {
