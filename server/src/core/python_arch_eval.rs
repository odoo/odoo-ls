...
impl PythonArchEval {
    ...
    fn resolve_narrowing_for_check(&mut self, session: &mut SessionInfo, check: &IsinstanceCheck, body_start: TextSize) {
        ...
        if evaluations.is_empty() {
            // Unresolved checked type (unimported, TYPE_CHECKING-only, a typo): point at the
            // shadowed declarations, so the narrowing is a no-op instead of erasing the type.
