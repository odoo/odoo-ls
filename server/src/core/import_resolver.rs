...
pub fn resolve_import_stmt(source_file_symbol: &Symbol, parent_symbol: &Symbol, from_stmt: Option<&Identifier>, name_aliases: &[Alias<TextRange>], level: Option<&Int>) -> &Symbol {
...
fn _get_or_create_symbol<'a>(symbol: &'a Symbol, names: &[String], file_symbol: &Symbol, asname: Option<String>) -> (Option<&'a Symbol>, &'a Symbol) {
    #TODO get arc from parent
    let mut sym: Option<&Symbol> = Some(symbol);
    let mut last_symbol = symbol;
    for branch in names {
        let mut next_symbol = symbol.get_symbol(vec![branch.clone()], vec![]);
        if next_symbol.is_none() {
            next_symbol = _resolve_new_symbol(???);
        }
        if next_symbol.is_none() {
            sym = None;
            break;
        }
        last_symbol = next_symbol.unwrap();
    }
