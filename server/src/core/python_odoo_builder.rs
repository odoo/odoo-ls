...
impl PythonOdooBuilder {
    ...
    fn _load_class_inherit(&mut self, session: &mut SessionInfo, symbol: &mut MainSymbol) {
    ...
    fn _load_class_name(&mut self, session: &mut SessionInfo, symbol: &mut MainSymbol) {
    ...
    fn test_symbol_is_model(&mut self, session: &mut SessionInfo, rc_symbol: &Rc<RefCell<MainSymbol>>, symbol: &mut MainSymbol) -> bool {
