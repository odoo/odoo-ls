...
use crate::core::symbols::symbol_table::{all_members, follow_ref, get_member_symbol, get_sym, is_field_class, ClassKey, SymbolKey};
...
impl PythonOdooBuilder {
    ...
    pub fn load(&mut self, session: &mut SessionInfo) -> Vec<Diagnostic> {
        ...
        let model_name = st!().classes[sym]._model.as_ref().unwrap().name.clone();
        if let Some(module) = st!().find_module(sym) {
            ...
            let set = st!().packages[module.unwrap_package_key()].as_module_package_mut().xml_id_locations.entry(xml_id_model_name.clone()).or_insert_with(HashSet::new);
            set.insert(file);
    ...
    fn _evaluate_name(&mut self, session: &mut SessionInfo, diagnostics: &mut Vec<Diagnostic>) -> OYarn {
        ...
        if let Some(inherit_name) = st!().classes[symbol]._model.as_ref().unwrap().inherit.first() {
            return inherit_name.clone();
        }
        st!().classes[symbol].name.clone()
    }
    ...
    fn _load_class_inherits(&mut self, session: &mut SessionInfo, diagnostics: &mut Vec<Diagnostic>) {
        ...
        if let Some(&_inherits) = _inherits.last() {
            for eval in st!().evaluations(_inherits).unwrap().clone() {
                let eval = eval.follow_ref_and_get_value(session, &mut None, diagnostics);
                let model = st!().classes[symbol]._model.as_mut().unwrap();
            ...
            for (symbol, _deps) in symbols.iter() {
                ...
                for eval in evals.clone() {
                    ...
                    if delegate.as_bool() == true && let Some(comodel) = context.get("comodel_name") {
                        ...
                        st!().classes[self.symbol]._model.as_mut().unwrap().inherits.push((comodel_name, field_name.clone()));
                    }
                }
            }
        }
    ...
    fn _load_class_attributes(&mut self, session: &mut SessionInfo, diagnostics: &mut Vec<Diagnostic>) {
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::StringLiteral(s))) = descr {
            st!().classes[symbol]._model.as_mut().unwrap().description = S!(s.value.to_str());
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().description = st!().classes[symbol]._model.as_ref().unwrap().name.to_string();
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::BooleanLiteral(b))) = auto {
            st!().classes[symbol]._model.as_mut().unwrap().auto = b.value;
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().auto = false;
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::BooleanLiteral(b))) = log_access {
            st!().classes[symbol]._model.as_mut().unwrap().log_access = b.value;
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().log_access = st!().classes[symbol]._model.as_ref().unwrap().auto;
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::StringLiteral(s))) = table {
            st!().classes[symbol]._model.as_mut().unwrap().table = S!(s.value.to_str());
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().table = st!().classes[symbol]._model.as_ref().unwrap().name.replace(".", "_");
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::StringLiteral(s))) = sequence {
            st!().classes[symbol]._model.as_mut().unwrap().sequence = S!(s.value.to_str());
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().sequence = st!().classes[symbol]._model.as_ref().unwrap().table.clone() + "_id_seq";
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::BooleanLiteral(b))) = is_abstract {
            st!().classes[symbol]._model.as_mut().unwrap().is_abstract = b.value;
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().is_abstract = true;
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::BooleanLiteral(b))) = transient {
            st!().classes[symbol]._model.as_mut().unwrap().transient = b.value;
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().transient = false;
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::StringLiteral(s))) = rec_name {
            st!().classes[symbol]._model.as_mut().unwrap().rec_name = Some(S!(s.value.to_str()));
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().rec_name = Some(S!("name")); //TODO if name is not on model, take 'id'
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::BooleanLiteral(b))) = _check_company_auto {
            st!().classes[symbol]._model.as_mut().unwrap().check_company_auto = b.value;
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().check_company_auto = false;
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::StringLiteral(s))) = parent_name {
            st!().classes[symbol]._model.as_mut().unwrap().parent_name = S!(s.value.to_str());
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().parent_name = S!("parent_id");
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::BooleanLiteral(b))) = parent_store {
            st!().classes[symbol]._model.as_mut().unwrap().parent_store = b.value;
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().parent_store = false;
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::StringLiteral(s))) = active_name {
            st!().classes[symbol]._model.as_mut().unwrap().active_name = Some(S!(s.value.to_str()));
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().active_name = None;
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::StringLiteral(s))) = data_name {
            st!().classes[symbol]._model.as_mut().unwrap().data_name = S!(s.value.to_str());
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().data_name = S!("date");
        }
        ...
        if let Some(EvaluationValue::CONSTANT(Expr::StringLiteral(s))) = fold_name {
            st!().classes[symbol]._model.as_mut().unwrap().fold_name = S!(s.value.to_str());
        } else {
            st!().classes[symbol]._model.as_mut().unwrap().fold_name = S!("fold");
        }
    }
    ...
    fn _add_magic_fields(&mut self, session: &mut SessionInfo) {
        ...
        //id
        let range = st!().classes[symbol].range.clone();
        ...
        if let Some(&id_field) = id_field.last() {
            ...
            st!().variables[id].evaluations.push(evaluation);
        }
        //display_name
        ...
        if let Some(&char_field) = char_field.last() {
            ...
            st!().variables[display_name].evaluations.push(evaluation);
        }
        //if log_access
        if st!().classes[symbol]._model.as_ref().unwrap().log_access {
            //create_uid
            ...
            if let Some(&many2one_field) = many2one_field.last() {
                ...
                st!().variables[create_uid].evaluations.push(evaluation);
            }
            //create_date
            ...
            if let Some(&datetime_field) = datetime_field.last() {
                ...
                st!().variables[create_date].evaluations.push(evaluation);
            }
            //write_uid
            ...
            if let Some(&many2one_field) = many2one_field.last() {
                let evaluation = Evaluation::eval_from_symbol(&st!(), many2one_field, Some(true));
                st!().variables[write_uid].evaluations.push(evaluation);
            }
            //write_date
            ...
            if let Some(&datetime_field) = datetime_field.last() {
                let evaluation = Evaluation::eval_from_symbol(&st!(), datetime_field, Some(true));
                st!().variables[write_date].evaluations.push(evaluation);
            }
        }
    }
            ...
            for eval in evals.clone() {
                ...
                for eval_ptr in eval_ptrs.iter() {
                    ...
                    if let Some(ContextValue::STRING(compute_ctx_val)) = eval_weak.context.get("compute") {
                        ...
                        st!().classes[symbol]._model.as_mut().unwrap().computes.entry(oyarn!("{}", compute_ctx_val)).or_insert_with(HashSet::new).insert(name);
                    }
                }
            }
        }
