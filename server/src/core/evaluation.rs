...
impl Evaluation {
    ...
    pub fn new_self(base: impl Into<Weak<SymbolKey>>, instance: Option<bool>) -> Self {
        let base = base.into();
        Self {
    ...
    pub fn analyze_ast(session: &mut SessionInfo, ast: &ExprOrIdent, parent: SymbolKey, max_infer: &TextSize, context: &mut Option<Context>, for_annotation: bool, required_dependencies: &mut Vec<Vec<SymbolKey>>) -> AnalyzeAstResult {
        ...
        match ast {
            ...
            ExprOrIdent::Expr(Expr::Lambda(lambda_expr)) => {
                let lambda_sym = st!().get_positioned_symbol(parent, &Sy!("<lambda>"), &lambda_expr.range);
                if let Some(lambda_sym) = lambda_sym {
                    if is_in_validation || session.sync_odoo.evaluation_search.is_some() {
                        if is_in_validation {
                            st!().set_build_status(lambda_sym, BuildSteps::VALIDATION, BuildStatus::IN_PROGRESS);
                        }
                        ...
                        if is_in_validation {
                            st!().set_build_status(lambda_sym, BuildSteps::VALIDATION, BuildStatus::DONE);
                            diagnostics.extend(diags);
                        }
                    }
                    evals.push(Evaluation::eval_from_symbol(&st!(), lambda_sym, None));
                };
            },
        ...
        if let Some(evaluation_search) = session.sync_odoo.evaluation_search.as_ref() {
            for eval in evals.iter() {
                ...
                if eval.symbol.sym.has_weak() && let Some(weak) = eval.symbol.sym.get_weak().weak.upgrade(&st!()) {
...
impl EvaluationSymbol {
    ...
    pub fn new_self(get_symbol_hook: Option<GetSymbolHook>, base: Weak<SymbolKey>, instance: Option<bool>) -> EvaluationSymbol {
        Self {
    ...
    pub fn get_symbol_weak_transformed(&self, session: &mut SessionInfo, context: &mut Option<Context>, diagnostics: &mut Vec<Diagnostic>, scope: Option<SymbolKey>) -> EvaluationSymbolPtr {
        ...
        match eval {
            ...
            EvaluationSymbolPtr::SELF(_) => {
                let default = EvaluationSymbolPtr::WEAK(EvaluationSymbolWeak{weak: Weak::null(), context: HashMap::new(), instance: Some(false), is_super: false});
