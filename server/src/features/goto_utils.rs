...
use crate::{
    ...
    features::{
        ...
        xml_ast_utils::{XmlAstUtils, GetSymbolsResult},
    },
    oyarn,
    threads::SessionInfo,
    utils::PathSanitizer,
...
impl GotoUtils {
    ...
    fn check_for_model_string(session: &mut SessionInfo, eval: &Evaluation, file_symbol: SourceFileKey, sources: &mut Vec<GotoSource>) -> bool {
        ...
        for class_key in classes {
            ...
            let path = session.st().path(file_symbol).to_string();
            sources.push(GotoSource {
                source: GotoSourceType::SymbolKey(class_key.into()),
    ...
    pub fn get_symbols_xml(session: &mut SessionInfo,
        ...
        if let Ok(document) = document {
            ...
            for xml_ast_result in xml_ast_results {
                match xml_ast_result {
                    GetSymbolsResult::SymbolKey(symbol_key) => {
                        sources.push({
                            let path = session.sync_odoo.symbol_table.path(file_symbol).to_string();
                            GotoSource {
                                source: GotoSourceType::SymbolKey(symbol_key),
                    ...
                    GetSymbolsResult::ComponentMember { component_member, uri } => {
                        sources.push({
                            let path = session.st().path(file_symbol).to_string();
                            GotoSource {
                                source: GotoSourceType::Location { uri, range: component_member.range },
                                origin_selection_range: Some(session.sync_odoo.get_file_mgr().borrow().std_range_to_range(session, &path, origin_range.as_ref().unwrap()))
                            }
                        });
                    },
                    GetSymbolsResult::TemplateVarDecl(template_var_decl) => {
                        sources.push({
                            let path = session.st().path(file_symbol).to_string();
                            GotoSource {
                                source: GotoSourceType::Location { uri: path.clone(), range: template_var_decl.range },
                                origin_selection_range: Some(session.sync_odoo.get_file_mgr().borrow().std_range_to_range(session, &path, origin_range.as_ref().unwrap()))
                            }
                        });
                    }
