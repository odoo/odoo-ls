...
impl AstUtils {
    ...
    pub fn get_symbols(odoo: &mut SyncOdoo, file_symbol: &Rc<RefCell<Symbol>>, file_info: &Rc<RefCell<FileInfo>>, offset: u32) -> (AnalyzeAstResult, Option<TextRange>) {
        ...
        let mut expr: Option<Either<&Expr, &Identifier>> = None;
        ...
        return match &expr {
            Either::Left(expr) => {
                let analyse_ast_result: AnalyzeAstResult = Evaluation::analyze_ast(odoo, Either::Left(expr), parent_symbol, &expr.range().end());
                (analyse_ast_result, Some(expr.range()))
            },
            Either::Right(ident) => {
                let analyse_ast_result: AnalyzeAstResult = Evaluation::analyze_ast(odoo, Either::Right(ident), parent_symbol, &ident.range().end());
                (analyse_ast_result, Some(ident.range()))
            }
        }
...
struct ExprFinderVisitor<'a> {
    offset: TextSize,
    expr: Option<Either<&'a Expr, &'a Identifier>>,
}

impl<'a> ExprFinderVisitor<'a> {

    pub fn find_expr_at(stmt: &'a Stmt, offset: u32) -> Option<Either<&Expr, &Identifier>> {
...
impl<'a> Visitor<'a> for ExprFinderVisitor<'a> {
    ...
    fn visit_expr(&mut self, expr: &'a Expr) {
        if expr.range().contains(self.offset) {
            ...
            if self.expr.is_none() {
                self.expr = Some(Either::Left(expr));
            }
        } else {
            walk_expr(self, expr);
        }
        ...
        if self.expr.is_none() {
            ...
            if let Some(ref ident) = handler.name {
                if ident.clone().range().contains(self.offset) {
                    self.expr = Some(Either::Right(&ident));
                }
            }
        } else {
            walk_except_handler(self, except_handler);
    ...
    fn visit_parameter(&mut self, parameter: &'a Parameter) {
        ...
        if self.expr.is_none() && parameter.name.range().contains(self.offset) {
            self.expr = Some(Either::Right(&parameter.name));
        }
    }
    ...
    fn visit_pattern_keyword(&mut self, pattern_keyword: &'a PatternKeyword) {
        ...
        if self.expr.is_none() && pattern_keyword.clone().attr.range().contains(self.offset) {
            self.expr = Some(Either::Right(&pattern_keyword.attr));
        } else {
            walk_pattern_keyword(self, pattern_keyword);
        }
    }
        ...
        if type_param.range().contains(self.offset) {
            if self.expr.is_none() {
                ...
                if ident.is_some() && ident.unwrap().range().contains(self.offset) {
                    self.expr = Some(Either::Right(ident.unwrap()));
                }
