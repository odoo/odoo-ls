...
use crate::core::file_mgr::{AstKind, FileInfoKey, FileMgr};
...
impl DefinitionFeature {
    ...
    fn get_js_definition(session: &mut SessionInfo, file_info: FileInfoKey, line: u32, character: u32) -> Option<GotoDefinitionResponse> {
        ...
        let template_refs: Vec<_> = session.sync_odoo.component_mgr.template_refs_by_file(&session.file_mgr()[file_info].uri.clone()).cloned().collect();
