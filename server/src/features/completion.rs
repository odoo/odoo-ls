                    ...
                    if model_name.starts_with(prefix) && model_name != "_unknown" {
                        ...
                        if let Some(ref current_module) = current_module {
                            ...
                            let required_modules = modules.filter(|module|
                                !ModuleSymbol::is_in_deps(session, &current_module, &module.borrow().as_module_package().name, &mut None));
                            let dep_names: Vec<String> = required_modules.map(|module| module.borrow().as_module_package().name.clone()).collect();
