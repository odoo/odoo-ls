...
use crate::{S, constants::{OYarn, SymType}, core::{odoo::SyncOdoo, symbols::symbol_keys::{CsvFileKey, ModuleKey, SymbolKey}, xml_data::{OdooData, OdooDataRecord}}, oyarn, threads::SessionInfo};
...
impl CsvAstUtils {
    ...
    // @arena: maybe CsvFileKey for file symbol?
    /*
    pub fn get_symbols(session: &mut SessionInfo, file_symbol: SymbolKey, csv_reader: &mut Reader<&[u8]>, model_name: &OYarn, offset: usize) -> Vec<GotoSource> {
        todo!()
        let mut results = vec![];
    ...
    */
    ...
    /*
    // @arena todo: params types not really well thought beforehand
    fn get_symbols_in_record(session: &mut SessionInfo, offset: usize, headers: &Vec<OYarn>, record: &StringRecord, results: &mut Vec<GotoSource>, csv_symbol: CsvFileKey, model_name: OYarn, main_symbol: SymbolKey, module: Option<ModuleKey>) {
        todo!()
